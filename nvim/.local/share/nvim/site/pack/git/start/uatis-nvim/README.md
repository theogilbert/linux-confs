...
chunks it touched and none of the others; the fork point moving resets a
chunk only where its own lines moved; a chunk edited after it was read
stops being read when the edit is written, and is read again when it is
undone. `pane.auto_read`, `pane.remember_read` and
`highlight.read_recede` are the knobs.
...
delta LOC — added *and* removed — over the chunks that are read, rather
than files, because `3/20` says nothing true about a branch whose third
file is a nine-hundred-line generated blob; the `3/5` is files read
entirely. What fills it is the marks, not where the cursor is: arriving
at the last file of a branch is not having read the branch. Being a
window status line it obeys your `laststatus`; `list.progress = false`
turns it off.

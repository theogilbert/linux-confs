...
vim.keymap.set(
    ...
    { desc = "[S]earch by grep, focused on a single file[t]ype" }
)
vim.keymap.set(
    "n", "<leader>sa",
...
vim.keymap.set("n", "<leader>dC", require("utilities.dap").prompt_condition, { desc = "Set [C]onditional breakpoint" })
...
vim.keymap.set("n", "<leader>dB", dap.clear_breakpoints, { desc = "Erase [B]reakpoints" })
...
vim.keymap.set({"n", "v"}, "<leader>dE", require("utilities.dap").peek_string_value, { desc = "[E]valuate expression (multiline string)" })
...
local quick_term = require("utilities.quick_term")
vim.keymap.set("n", "<leader>gl", quick_term.runner({ cmd = "lazygit" }), { desc = "[G]it - Open [l]azygit" })
...
vim.keymap.set({ "n" }, "gs", sections.jump, { desc = "[G]o to [s]ection..." })
